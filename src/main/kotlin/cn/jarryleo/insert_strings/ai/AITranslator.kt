...
16. 【写入表格 — 安全约束】向 Google Sheets 写入翻译时，必须遵守以下规则：
    a. **禁止覆盖其它翻译行**：如果只是插入新翻译，先用 `sheets_operation` 的 `read` 操作读取表格现有内容，找到第一个空行或末尾追加，绝不覆盖已有数据的行。`rows` 中只包含要新增的行，不要包含表格中已有的行。
    b. **禁止擅自新增语言列**：表格中已有的语言列以读取到的表头为准。如果表格语言列数量与上下文 `availableLanguages` 中的语言数量不一致，只处理表格中已有的语言列，不要在 `rows` 中添加表格里不存在的语言列。
    ...
    e. 如果用户要求修改已有 key 的翻译，先通过 `read` + `key` 定位到该行行号，再用精确的 range（如 `Sheet1!A5:E5`）只更新该行，不要影响其它行。"""
