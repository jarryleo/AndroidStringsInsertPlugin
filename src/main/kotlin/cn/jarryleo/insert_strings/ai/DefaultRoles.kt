 ...
 * 该角色才会真正生效。
 *
 * 与 [cn.jarryleo.insert_strings.phrases.DefaultPhrases] 的区别:
 * - phrases 的默认值有完整 text(常用操作模板);
 * - roles 的默认值没有 prompt(角色风格完全由用户决定,默认值给出来反而会限制创意)。
 *
 * 写入时机:由 [cn.jarryleo.insert_strings.ui.InsertStringsRolesController.loadRoles] 在
 * [cn.jarryleo.insert_strings.ai.AiRolesState.defaultsInstalled] = false 时调用
 * [cn.jarryleo.insert_strings.ai.AiRolesService.installDefaultsIfNeeded] 写入,
