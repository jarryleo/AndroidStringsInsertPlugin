...
import cn.jarryleo.android.buddy.ai.TodoUiRefresher.refresh
import cn.jarryleo.android.buddy.ai.TodoUiRefresher.refreshImmediate
import cn.jarryleo.android.buddy.ai.TodoUiRefresher.setRefresher
import cn.jarryleo.android.buddy.ui.TodoReminderPopup
...
class TodoReminderScheduler : Disposable {
     ...
     *
     * 使用 `NotificationGroupManager` 获取在 [plugin.xml] 注册的 `Android Buddy Todo Reminders` 组,
    ...
    private fun showReminderBalloon(item: TodoItem, aiResponse: String) {
        ...
        try {
            val group = NotificationGroupManager.getInstance()
                .getNotificationGroup("Android Buddy Todo Reminders")
            if (group != null) {
                ...
                log.warn("TodoReminderScheduler: notification group 'Android Buddy Todo Reminders' not found, fallback to Notifications.Bus")
                val notification = com.intellij.notification.Notification(
                    "Android Buddy Todo Reminders",
                    titleText,
                    content,
                    NotificationType.INFORMATION
                )
     ...
     * [cn.jarryleo.android.buddy.ui.InsertStringsTodosController] 暴露的 service-level 钩子,
     * 在 UI 装配时由 [cn.jarryleo.android.buddy.ui.InsertStringsUI.createToolWindowContent] 注册。
     *
