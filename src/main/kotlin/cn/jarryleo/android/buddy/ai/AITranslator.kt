...
import cn.jarryleo.android.buddy.ai.AITranslator.CHAT_COMMON_RULES
import cn.jarryleo.android.buddy.ai.AITranslator.MAIN_PANEL_SYSTEM_PROMPT
import cn.jarryleo.android.buddy.ai.AITranslator.QUOTE_ENTRY_SYSTEM_PROMPT
import cn.jarryleo.android.buddy.ai.AITranslator.anthropicChatBody
import cn.jarryleo.android.buddy.ai.AITranslator.buildAnthropicMessages
import cn.jarryleo.android.buddy.ai.AITranslator.chat
import cn.jarryleo.android.buddy.ai.AITranslator.extractAssistantText
import cn.jarryleo.android.buddy.ai.AITranslator.extractReasoningText
import cn.jarryleo.android.buddy.ai.AITranslator.extractToolCalls
import cn.jarryleo.android.buddy.ai.AITranslator.normalizeMessagesForAnthropic
import cn.jarryleo.android.buddy.ai.AITranslator.openAiChatBody
import cn.jarryleo.android.buddy.ai.AITranslator.parseAiReply
import cn.jarryleo.android.buddy.ai.AITranslator.parseRowColorMatrix
...
data class ChatMessage(
     ...
     *
     * 历史:之前 assistant 消息带真实 tool_calls 时,driver 把"执行操作: android.buddy / update_string"
...
- 同一 AI 回合内的所有 `android.buddy` / `update_string` / `delete_string` 写入必须在同一模块(全部省略 module,或全部显式指定同一 module),**不要**用项目名当 module;系统会兜底拦截并把错误回传给你。
...
- 不要用 `run_shell` 写 `strings.xml`、调 `git commit` / `git push` / 跑 `gradle assemble*` 这类有专用工具的操作;优先用专用工具(android.buddy / replace_selection / file_ops)。
...
- 字符串类型的参数**值**内部可以包含 markdown 语法(用于翻译文本的特殊格式),JSON 本身仍须合法。
  - 例:android.buddy 的 translations.values 可以是 `"**Hello**"`(字符串值含 markdown),不影响 JSON 解析,UI 端按 markdown 渲染。
...
- 插入翻译前若发现与现有 key 冲突,`options` **必须**使用以下统一格式(便于你后续按选项文本判断用户决策):
 ...
 ## 工具集(按需加载详细用法)
 - **strings.xml**:query_keys / read_string / find_keys_by_text / android.buddy / update_string / delete_string
...
- `android.buddy` 的 `translations` **必须**覆盖目标模块全部语种,缺一会让那个语言在 UI 上显示空串/key 名;`values`(默认英语)始终要包含。
...
4. 用户选择后的处理:
   - `使用现有 key:<existing_key>`:
     ...
     - **不要**再调 `android.buddy`(待插入的新 key 已被忽略)。
   - `插入新 key`:`query_keys` 查 key 名冲突,冲突则重新生成直到唯一;再调 `android.buddy`(主面板场景无选区,无需 replace_selection)。
...
- 这些动作**不调用任何工具**(android.buddy / update_string / replace_selection / read_string / find_keys_by_text 都不调),直接 markdown 回复即可,然后 `task_complete` 结束本轮。
...
4. 用户选择后的处理:
   - `使用现有 key:<existing_key>` —— ⚠️ **必须按顺序执行以下两步,不可跳过第一步** ⚠️:
     ...
     - **第二步**:`read_string(<existing_key>)` 取全语种翻译,逐项检查;若需修正先 `ask_user` 确认,得到肯定答复后用 `update_string` 精准补全;已完整则 `task_complete`。
     - **不要**再调 `android.buddy`(待插入的新 key 已被忽略)。
   - `插入新 key`:`query_keys` 查 key 名冲突则重新生成;再调 `android.buddy`(driver 会在成功后自动触发 `onInsertStringsInserted` 完成选区替换,无需你再调 `replace_selection`)。
   - `取消操作`:直接 `task_complete`。
...
- 精准子串场景:oldText = 选区里要换的那段,newText = 对 key 的引用;选区内多次出现会全部替换。"""
    ...
    /** 主面板聊天入口标识(由 [cn.jarryleo.android.buddy.ui.InsertStringsUI] 写入上下文)。 */
    ...
    private val TOOL_DOCS: Map<String, String> = mapOf(
        "android.buddy" to """
            ...
            - 若上下文有 currentKeys，优先用第一个 key 作为 name；多 key 时可为每个 key 分别返回 android.buddy。
            - 可以同时返回多个 android.buddy 动作插入多个字符串。
            - 你不用考虑转义,系统会自动转义。
            ...
            - **用户选择后的处理流程(由 AI 自行驱动,系统不再自动触发替换)**:
              - 选「使用现有 key:<existing_key>」—— ⚠️ **强约束:必须按顺序执行以下两步,不可跳过第一步** ⚠️:
                ...
                - **不要**再调用 android.buddy(那个待插入的新 key 已被忽略)。
              - 选「插入新 key」:用 query_keys 查你准备生成的 key 名是否已存在,若存在重新生成一个不冲突的 key(长度仍不超过 40 字符);
                然后调用 android.buddy(若来自布局/代码选区,driver 在写完后会自动触发 onInsertStringsInserted 完成硬编码文本的替换,无需你再调用 replace_selection)。
              - 选「取消操作」:无需任何处理,直接 task_complete 即可。
            - 一次 android.buddy 涉及多个 key 时,只要其中任何一个命中查重,整批都要用一次 ask_user 列出全部命中项,不可只对部分 key 跳过查重。
            示例（5 个语种全覆盖）:
            {"type":"android.buddy","module":"app","name":"hello_world","translations":{"values":"Hello","values-zh-rCN":"你好","values-ja":"こんにちは","values-ko":"안녕하세요","values-fr":"Bonjour"}}
        """.trimIndent(),
        ...
        "update_string" to """
            ...
            关键规则：
            - **只动 translations 中列出的语言**，未列出的语言保持原样（这是与 android.buddy 的最大区别）。
            - 若 key 在某语言文件中不存在则在该文件中创建条目。
            - 适用场景：「只改 X 的繁体」「修正 Z 的某个语言翻译」「新增某语言翻译」。
            - 不适用场景：要全量覆盖某 key 的全部语言翻译时，请改用 android.buddy。
        ...
        "find_keys_by_text" to """
            ...
            **固定全模块 + 全语言目录搜索**(2026.x 去掉 module / language 参数;原文查重的
            ...
            - **android.buddy 前必跑(查重入口)**:用**用户消息中的原始文本**(用户从布局/代码中选中的
        ...
        "code_ops" to """
            ...
            **第 5 步:检错(整文件改完后再做)** —— 对**同一文件**的一组 edit_file **全部跑完之后**,
            ...
            - 如果代码里的硬编码字符串应该提取为 strings.xml,先 android.buddy 走翻译查重,
        ...
        "ask_user" to """
            ...
            - **翻译查重专用格式(仅 AI 内部约定)**:检测到 android.buddy 与现有 key 重复时,
              ...
              **系统不再自动拦截**「使用现有 key」选项 — 用户点选后选项文本直接回传给你,
              ...
              - 选「插入新 key」:重新生成不冲突的 key 后调用 android.buddy(driver 会自动替换硬编码文本)。
        ...
        "run_shell" to """
            ...
            - 写 strings.xml 类的操作**不要**用 run_shell(有专用 android.buddy 工具),用专用工具更安全(模块一致性兜底)。
        ...
        "load_tool_doc" to """
            ...
            - tool（必填）：要加载的工具名。当前可用值：
              - android.buddy、query_keys、read_string、update_string、delete_string、find_keys_by_text
        ...
        val root = JsonObject().apply {
            ...
            add(
                ...
                JsonArray().apply {
                    ...
                    // 启用的 AI 角色:把 [cn.jarryleo.android.buddy.ai.AiRole.prompt] 注入到
