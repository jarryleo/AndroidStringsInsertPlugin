...
import cn.jarryleo.android.buddy.xml.KeySearchResult
import cn.jarryleo.android.buddy.xml.KeyTextSearchResult
import cn.jarryleo.android.buddy.xml.StringsService
...
internal class InsertStringsStringsOpsController(
     ...
     *    (见 [cn.jarryleo.android.buddy.xml.ContextInfo.recommendedDefaultModule]):
    ...
    fun runQueryKeys(action: AiAction.QueryKeys): String {
        val context = ContextManager.getInstance(project)
        val contextInfo = context.contextInfo
        // 默认走「推荐默认模块」,与 android.buddy 一致:用户没指定时优先 currentModule,
    ...
    private fun refreshKeyEntryAfterDelete(moduleName: String, key: String) {
        ...
        state.keyEntries[idx] = cn.jarryleo.android.buddy.xml.KeyedStringsInfo(key, "", updated)
