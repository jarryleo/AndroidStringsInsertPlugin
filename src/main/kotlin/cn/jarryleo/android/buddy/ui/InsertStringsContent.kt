...
internal fun InsertStringsContent(
     ...
     * nullable 入口 = 清除提醒。
     */
    onSaveTodoReminder: (cn.jarryleo.android.buddy.ai.TodoReminder?) -> Boolean,
    /**
     ...
     */
    pendingImages: List<cn.jarryleo.android.buddy.ai.ChatAttachment> = emptyList(),
    /**
     * 「📎」按钮回调(2026.x 多模态):弹文件选择器 / 把选中的图加入 [pendingImages]。
     */
    onPickImage: () -> Unit = {},
    /**
     * 单张图 × 删除回调(2026.x 多模态)。参数是 [cn.jarryleo.android.buddy.ai.ChatAttachment.id]。
     */
...
private fun MainTabBar(
    ...
    Row(
        ...
        // Chat 排在第一位,作为 Android Buddy 的「首页」tab。
        TabButton(
            text = "Chat",
        ...
        // Todo tab:位于 Translations 与 Settings 之间,展示代办列表(主面板专属)。
