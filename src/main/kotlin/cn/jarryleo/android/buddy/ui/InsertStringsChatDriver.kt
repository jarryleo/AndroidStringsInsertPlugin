...
import cn.jarryleo.android.buddy.ai.AiReply
import cn.jarryleo.android.buddy.ai.ChatAttachment
import cn.jarryleo.android.buddy.ai.ChatMessage
import cn.jarryleo.android.buddy.ai.RetrySupport
import cn.jarryleo.android.buddy.ai.ToolCall
import cn.jarryleo.android.buddy.ai.ToolDefinitions
...
internal class InsertStringsChatDriver(
     ...
     *
     * 用法:由 [cn.jarryleo.android.buddy.ai.TodoAiResponder] 调用 —— 用户代办提醒触发时,
     ...
     *  - 「插入新 key」:AI 检查自己生成的 key 是否已存在,若存在则重新生成,然后调用 android.buddy;
    ...
    private fun processAiReply(
        ...
        // Priority 4-5: strings.xml 写操作(android.buddy / update_string / delete_string)统一做模块一致性校验
        ...
        if (insertEntries.isNotEmpty()) {
            ...
            addSkippedToolResults(unprocessed, "因已执行 android.buddy 而跳过")
            return
        }
    ...
    private fun addSkippedToolResults(
        ...
        unprocessedPairs.forEach { (action, toolCallId) ->
            ...
            val actionLabel = when (action) {
                is AiAction.InsertStrings -> "android.buddy"
    ...
    /**
     * 检测 write actions(android.buddy + update_string + delete_string)是否指定了不同模块。
    ...
    private fun rejectWriteModuleConflict(
        ...
        val errorMsg = buildString {
            append("[工具执行异常] 跨模块冲突:本轮内 android.buddy/update_string/delete_string 动作指定了多个不同 module(")
     ...
     * - 写操作:todo_add / todo_update / todo_delete 直接调 [cn.jarryleo.android.buddy.ai.TodoService];
     * - 读操作:todo_list 走 [cn.jarryleo.android.buddy.ai.TodoService.list] /
     *   [cn.jarryleo.android.buddy.ai.TodoService.listActive] / listCompleted 三种;
     *
     * 每个动作都返回 [cn.jarryleo.android.buddy.ai.TodoItem](读) / 操作结果(写)作为
     * tool_result,让 AI 在下一轮看到完整对象,可以做"读完改"、"改完再读"等连续动作。
     *
     * UI 同步:写操作完成后调 [cn.jarryleo.android.buddy.ui.InsertStringsTodosController]
    ...
    private fun executeTodoActions(
        ...
        val ui = state as? cn.jarryleo.android.buddy.ui.InsertStringsUI
        val service = cn.jarryleo.android.buddy.ai.TodoService.getInstance()
        entries.forEach { (action, toolCallId) ->
            when (action) {
                is AiAction.TodoList -> {
                    val items: List<cn.jarryleo.android.buddy.ai.TodoItem> = when (action.filter.lowercase()) {
                ...
                is AiAction.TodoAdd -> {
                    ...
                    if (title.isEmpty()) {
                        ...
                        val priority = cn.jarryleo.android.buddy.ai.TodoPriority.fromName(action.priority)
                        ...
                        val newItem = cn.jarryleo.android.buddy.ai.TodoItem(
                            title = title,
                        ...
                        if (reminder != null) {
                            cn.jarryleo.android.buddy.ai.TodoReminderScheduler.getInstance()
                ...
                is AiAction.TodoUpdate -> {
                    ...
                    if (current == null) {
                        ...
                        val newPriority = if (action.priority != null) {
                            cn.jarryleo.android.buddy.ai.TodoPriority.fromName(action.priority)
                        ...
                        val newReminder: cn.jarryleo.android.buddy.ai.TodoReminder? = when {
                        ...
                        val scheduler = cn.jarryleo.android.buddy.ai.TodoReminderScheduler.getInstance()
                        when {
    ...
    /**
     * 把 AI 传入的 reminder 参数合成为 [cn.jarryleo.android.buddy.ai.TodoReminder]。
     *
     * 优先级(从高到低):
    ...
    private fun buildReminderFromAiArgs(
    ...
    ): cn.jarryleo.android.buddy.ai.TodoReminder? {
        ...
        if (parsedDate != null) {
            ...
            val tod = cn.jarryleo.android.buddy.ai.TodoTimeOfDay(hour, minute)
            return cn.jarryleo.android.buddy.ai.TodoReminder(
                enabled = true,
                nextTriggerAt = ts,
                recurrence = cn.jarryleo.android.buddy.ai.TodoRecurrence.NONE,
                timeOfDay = tod,
        ...
        if (reminderTime != null) {
            val rec = cn.jarryleo.android.buddy.ai.TodoRecurrence.fromName(recurrence)
            val tod = cn.jarryleo.android.buddy.ai.TodoReminder.deriveDefaultsFrom(reminderTime).first
            val days = if (rec == cn.jarryleo.android.buddy.ai.TodoRecurrence.CUSTOM) {
        ...
        if (rec == cn.jarryleo.android.buddy.ai.TodoRecurrence.CUSTOM && days.isEmpty()) {
            return null
        }
        if (rec == cn.jarryleo.android.buddy.ai.TodoRecurrence.NONE) {
        ...
        val allowedDays: Set<Int> = when (rec) {
            cn.jarryleo.android.buddy.ai.TodoRecurrence.DAILY -> emptySet() // 空集 = 任意 day 都行
            cn.jarryleo.android.buddy.ai.TodoRecurrence.CUSTOM -> days
            else -> emptySet()
        }
    ...
    /**
     * 把 [cn.jarryleo.android.buddy.ai.TodoReminder] 序列化为简短 JSON,供 AI 工具结果回传。
     *
     * 字段(2026.x 增强):
     ...
     */
    private fun reminderToJsonString(r: cn.jarryleo.android.buddy.ai.TodoReminder): String {
        val tod = r.timeOfDay?.format() ?: "-"
        val days = if (r.recurrence == cn.jarryleo.android.buddy.ai.TodoRecurrence.CUSTOM) {
    ...
    private fun executeInsertActions(
            ...
            entries.forEach { (_, toolCallId) ->
                if (toolCallId.isNotEmpty()) {
                    state.chatMessages.add(
                        ChatMessage(
                            role = "tool",
                            content = "[工具执行异常] android.buddy 未指定目标模块且无 currentModule , moduleList = $moduleList",
                            toolCallId = toolCallId
                        )
                    )
                }
            ...
            entries.forEach { (_, toolCallId) ->
                if (toolCallId.isNotEmpty()) {
                    state.chatMessages.add(
                        ChatMessage(
                            role = "tool",
                            content = "[工具执行异常] android.buddy 模块名称 = 项目名称,请勿使用项目名插入翻译 , moduleList = $moduleList",
                            toolCallId = toolCallId
                        )
                    )
                }
        ...
        // - AI 在调用 android.buddy 之前应已用 find_keys_by_text 扫过现有 key,若发现重复会主动用
        ...
        val results = actions.map { action ->
            ...
            // 避免 android.buddy 覆写带 HTML 的翻译时把包裹一起吞掉(丢了之后
            // Android 端会按字面量渲染 <,lint 也会报 unescaped <)。详见
            // [cn.jarryleo.android.buddy.xml.AndroidStringEscaper.preserveWrapping]。
            action.translations.forEach { (lang, aiText) ->
                val existing = merged[lang]
                merged[lang] = if (existing != null) {
                    cn.jarryleo.android.buddy.xml.AndroidStringEscaper
            ...
            if (targetModuleLanguages.isNotEmpty()) {
                ...
                if (filledLanguages.isNotEmpty() || droppedLanguages.isNotEmpty()) {
                    DebugLog.log(
                        ...
                        "android.buddy key=${action.name} module=$targetModule " +
        ...
        // 通知 state 有 android.buddy 已完成(供 ExtractStringsChatHolder 这类入口
        ...
        // 为每个 android.buddy 调用添加对应的 tool result(使用 entry 自带的 toolCallId,避免下标错位)
        entries.forEachIndexed { i, (_, toolCallId) ->
            ...
            if (action != null && toolCallId.isNotEmpty()) {
                state.chatMessages.add(
                    ChatMessage(
                        role = "tool",
                        content = "[工具执行结果] android.buddy module=$batchModule name=${action.name} 状态:$msg",
                        toolCallId = toolCallId
                    )
                )
            }
