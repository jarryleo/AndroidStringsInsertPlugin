 ...
 *  - 自动替换:driver 走完 android.buddy 后回调 [ChatStateHolder.onInsertStringsInserted],
...
internal class InsertStringsEditorOpsController(
        ...
        return synchronized(replaceLock) {
            if (state.editorReplacementTriggered) {
                return@synchronized "[工具执行结果] 类型:replace_selection 状态:已跳过 " +
                    "信息:编辑器选区已被前序自动替换处理(android.buddy 触发的 onInsertStringsInserted " +
     ...
     *
     * 替换策略(与 [cn.jarryleo.android.buddy.ExtractStringsAction] 原
    ...
    companion object {
         ...
         *
         * 翻译查重 +「使用现有 key」流程:driver 走完 android.buddy 后回调
         ...
         * 与 [cn.jarryleo.android.buddy.ExtractStringsAction.isLayoutXmlFile] 保持一致。
         */
