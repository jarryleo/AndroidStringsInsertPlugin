...
import cn.jarryleo.android.buddy.ui.*
 ...
 * 3. AI 完成 android.buddy / 复用 existing key 后:
 *    - 拿到刚写入的 key(由 [InsertStringsChatDriver] 通过
 *      [ChatStateHolder.onInsertStringsInserted] 回调传回)
 *    - 把编辑器选区替换成引用(委托给 [cn.jarryleo.android.buddy.ui.InsertStringsEditorOpsController]):
 ...
 *    [cn.jarryleo.android.buddy.ui.InsertStringsEditorOpsController.runReplaceSelection]
...
class ExtractStringsAction : AnAction() {
     ...
     * [cn.jarryleo.android.buddy.ui.InsertStringsEditorOpsController](见
     * [cn.jarryleo.android.buddy.ui.InsertStringsEditorOpsController.runReplaceSelection]
     * 与 [cn.jarryleo.android.buddy.ui.InsertStringsEditorOpsController.performReplace])。
 ...
 * - 持有一个 [cn.jarryleo.android.buddy.ui.InsertStringsEditorOpsController.runReplaceSelection]
...
private class ExtractStringsChatHolder(
    ...
    /**
     * 注入「driver 完成 android.buddy 后自动触发的替换回调」。
     * 由 [ExtractStringsAction.showExtractDialog] 在构造完
     * [cn.jarryleo.android.buddy.ui.InsertStringsEditorOpsController] 后调用,
     ...
     * 替换为对 key 的引用。
     *
     * 替换 / 防双重替换 / 弹框保持打开等逻辑全部委托给 [cn.jarryleo.android.buddy.ui.InsertStringsEditorOpsController]
